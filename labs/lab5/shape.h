...
#include <cstdlib>
#include <vector>
#include "C:/Users/gross/Documents/GitHub/csci441-spring2018/labs/lib/csci441/include/csci441/vector.h"
...
void add_vertex(T& coords, const N& x, const N& y, const N& z,
                const C& r, const C& g, const C& b, float x1, float y1, float z1 ,bool with_noise=false) {
    // adding color noise makes it easier to see before shading is implemented
    float noise = 1-with_noise*(rand()%150)/100.;
    coords.push_back(x);
    coords.push_back(y);
...
//        vector<triangle> triangleList;
        ...
        vector<float> vertex;
        for(int r = 0; r<coords.size()/6; r+=6){
            vertex.clear();
            vertex.push_back(coords.at(r));
            vertex.push_back(coords.at(r + 1));
            vertex.push_back(coords.at(r + 2));
    ...
    Sphere(unsigned int n, float radius, float r, float g, float b) {
        ...
        for (int i = 0; i < n_steps; ++i) {
            for (int j = 0; j < n_steps/2.0; ++j) {
                ...
                // add triangle
                add_vertex(coords, vij_x, vij_y, vij_z, r, g, b,  vij_x, vij_y, vij_z);
                add_vertex(coords, vip1j_x, vip1j_y, vip1j_z, r, g, b, vip1j_x, vip1j_y, vip1j_z);
                add_vertex(coords, vijp1_x, vijp1_y, vijp1_z, r, g, b, vijp1_x, vijp1_y, vijp1_z);

                // add triange
                add_vertex(coords, vijp1_x, vijp1_y, vijp1_z, r, g, b, vijp1_x, vijp1_y, vijp1_z);
                add_vertex(coords, vip1jp1_x, vip1jp1_y, vip1jp1_z, r, g, b, vip1jp1_x, vip1jp1_y, vip1jp1_z);
                add_vertex(coords, vip1j_x, vip1j_y, vip1j_z, r, g, b, vip1j_x, vip1j_y, vip1j_z);
            }
        }
    }
