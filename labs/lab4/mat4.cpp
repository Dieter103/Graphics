...
///outputs a 4x4 translation matrix
...
mat4 mat4::lookAt(vec3 eye, vec3 target, vec3 upDir){
    vec3 forward = eye - target;
    forward = forward.normalize(forward);
    std::cout << forward << std::endl;
    vec3 left = upDir.cross(upDir, forward);
    std::cout << left << std::endl;
    left = left.normalize(left);

    vec3 up = left.cross(forward, left);
    std::cout << up << std::endl;

    mat4 matrix;
    matrix = matrix.identity();

    matrix[0] = vec4(left.x, left.y, left.z, -left.x * eye.x - left.y * eye.y - left.z * eye.z);
    matrix[1] = vec4(up.x, up.y, up.z, -up.x * eye.x - up.y * eye.y - up.z * eye.z);
    matrix[2] = vec4(forward.x, forward.y, forward.z, -forward.x * eye.x - forward.y * eye.y - forward.z * eye.z);
    return matrix;
}

mat4 mat4::prjectionMatrix( const float &b, const float &t, const float &l,
                             const float &r, const float &n, const float &f){
    mat4 m = mat4();

    m[0] = vec4(2 * n / (r - 1), 0 , (r + 1) / (r - 1), 0);
...
mat4 mat4::perspMatrix( const float ar, const float near, const float far, float fov){

    float fieldOfView = tan(convertDegToRad(fov/2));
    float range = near - far;
    mat4 m = mat4();

    m[0] = vec4(1/(fieldOfView*ar), 0,0,0);
    m[1] = vec4(0,1/fieldOfView, 0, 0);
    m[2] = vec4(0, 0, (-near - far)/range, 0);
    m[3] = vec4(0, 0, 2*far*near/range, 0);
